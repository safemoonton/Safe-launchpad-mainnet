...
import icoHex from "../contracts/jetton-ico.compiled";
...
function trimNumber(num) {
    ...
    const jettonIco = JettonIco.createFromConfig(
      {
        ...
        platformOwnerPercentage: PRIVATE_PLATFORM_FEE,
        rate: rate,
      ...
      if (result?.boc !== undefined) {
        ...
        if (ico) {
          setTimeout(() => {
            ...
            navigate(`/safe-launch/private/${ico._id}`);
          }, 5000);
        }
      }
    } catch (err) {
  ...
  const parameterChecks = () => {
    if (formStep === 0) {
      ...
      if (
        ...
        if (Number(soft_cap) > Number(hard_cap)) {
          ...
          setFormData({
            ...formData,
            token_price: (Number(hard_cap) / Number(total_token_sale)).toFixed(
              10
            ),
          });
          setFormStep(1);
    ...
    if (formStep == 1) {
      ...
      const required_tokens = hard_cap / token_price + Number(lp_jettons);
      ...
      } else if (listingType == "auto" && lp_jettons == "") {
      ...
      } else if (Number(required_tokens) > Number(total_token_sale)) {
        toast.error(
          `${required_tokens} ${tokenData.symbol} required! Update your Total token sale`,
          {
            position: "top-right",
            autoClose: 5000,
            hideProgressBar: false,
          ...
          <ol className="hidden lg:flex items-center w-full p-3 space-x-2 text-sm font-medium text-center text-gray-500 bg-white border border-gray-200 rounded-2xl shadow-sm dark:text-gray-400 sm:text-base dark:bg-black dark:border-gray-700 sm:p-4 sm:space-x-4 rtl:space-x-reverse">
            ...
            <li
              className={`flex items-center ${
                ...
                formStep === 2 ||
                formStep === 3 ||
                (formStep === 4 && "text-safemoon-dark dark:text-white")
              }`}
            >
              <span
                className={`flex items-center justify-center w-5 h-5 me-2 text-xs border rounded-full shrink-0 ${
                ...
                <>
                  {tokenVerify && (
                    <div className="mt-4">
                      <div className="mb-4 xl:flex md:gap-4 md:w-full md:flex-wrap">
                        <div className="mb-4 xl:mb-0 xl:w-full">
                          ...
                          <input
                            type="number"
                            id="total_token_sale"
                            name="total_token_sale"
                            className="shadow appearance-none border rounded-2xl text-sm w-full py-2 pr-2 pl-4 text-gray-700 leading-tight focus:outline-none"
                            value={formData.total_token_sale}
                            onChange={handleInputChange}
                          />
                        </div>
                      </div>
                      ...
                      {formData.total_token_sale !== 0 &&
                        formData.total_token_sale !== null &&
                        formData.hard_cap !== 0 &&
                          ...
                          <div className="flex gap-8 justify-center items-center my-16">
                            ...
                            <h1 className="text-white dark:text-gray-300 text-3xl font-bold tracking-wide uppercase">
                              ...
                              {(
                                ...
                                Number(formData.total_token_sale)
                              ).toFixed(10)}{" "}
                              TON
                            </h1>
                          </div>
              ...
              <div className="mb-4">
                ...
                <div className="flex items-center mb-1">
                  <input
                    ...
                    name="launchpadType"
                    value="private"
                    checked={formData.launchpadType === "private"}
                    onChange={() => handleLaunchpadTypeChange("private")}
              ...
              <div className="mb-4">
                ...
                <div className="flex items-center mb-1">
                  <input
                    ...
                    name="listingType"
                    value="manual"
                    checked={formData.listingType === "manual"}
                    onChange={() => handleListingTypeChange("manual")}
                    className="w-4 h-4  bg-gray-100 border-gray-300 focus:none dark:focus:none dark:ring-transparent focus:ring-transparent dark:bg-gray-700 dark:border-gray-600"
                  />
                  <label
                    htmlFor="manual"
