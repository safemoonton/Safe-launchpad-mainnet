...
import LaunchpadAdmin from "../components/LaunchpadAdmin";
...
const PrivateLaunchpadDetails = () => {
  ...
  async function updateWhitelistData(whitelistType, whitelist) {
    let newFormData = {
      ...launchpadData,
      whitelistType: whitelistType,
      ...(whitelistType === "basic"
        ? { basicWhitelist: whitelist }
        : { tieredWhitelist: whitelist }),
    };
    ...
    <>
                        ...
                        {walletAddress ? (
                          <>
                            {isAddressWhitelisted(
                              walletAddress,
                              launchpadData.start,
                              launchpadData.whitelistType,
                              launchpadData.basicWhitelist,
                              launchpadData.tieredWhitelist
                            ) ? (
                              <div>
                              ...
                              <div className="mt-4 flex justify-center items-center">
                                <p>
                                  Your Address is not whitelisted or not in
                                  current whitelist tier. Check back later
                                </p>
                              </div>
                            )}
                          </>
                                ...
                                Number(launchpadData.soft_cap) ? (
                                  ...
                                  <>
                                    <p className="mt-4">
                                      ...
                                      {launchpadData.soft_cap} TON. Get your
                                      refund
                                    </p>
                                    {myContribution.length > 0 &&
                                      myContribution.map(
      ...
      {modalType === "whitelist" && (
        <SetWhitelistModal
          whitelistData={
            {
              basic: launchpadData?.basicWhitelist,
              tiered: launchpadData.tieredWhitelist,
            } || ""
          }
          type={launchpadData?.whitelistType || "basic"}
          closeModal={handleModal}
          updateWhitelistData={updateWhitelistData}
        />
      )}
      {modalType === "delete" && (
        <SetDeleteModal
...
const SetDeleteModal = ({ closeModal, type, updateWhitelistData }) => {
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      if (type == "basic") {
        await updateWhitelistData("basic", "");
      } else {
        await updateWhitelistData("tiered", []);
      }

      toast.success("All whitelists removed", {
        position: "top-right",
        autoClose: 3000,
        hideProgressBar: false,
        closeOnClick: true,
      ...
      toast.error("Error removing whitelists");
    }
  };
    ...
    <div className="fixed inset-0 flex items-start justify-center bg-gray-600 bg-opacity-75 z-50">
      <div className="text-safemoon-alt  px-4 md:px-4 dark:bg-[#1c1c1c] dark:text-white bg-safemoon-dark text-white p-6 rounded-2xl shadow-md max-w-xl w-[90%] mt-20 z-[1000]">
        ...
        <p className="mb-4 tracking-wide text-sm">
          Are you sure you want to delete all whitelists for this sale?
        </p>
...
export default PrivateLaunchpadDetails;
