...
import { normalizeAddress } from "../helpers/index";
...
const PrivateSales = () => {
  ...
  async function getData() {
    try {
      ...
      const data = response.reverse().map((launchpad) => {
        const start = new Date(launchpad.start).getTime();
        const end = new Date(launchpad.end).getTime();
      ...
      const filteredData = data.filter(
        (launchpad) => launchpad.launchpadType == "private"
      );
  ...
  const filterLaunchpads = () => {
    ...
    if (filter === "My") {
      filteredData = filteredData.filter(
        (launchpad) =>
          normalizeAddress(launchpad.creatorAddress) ===
      ...
      <div className="my-4">
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-2">
          <div className="">
            ...
            <p className="mb-3 max-w-[30rem] text-sm font-light text-gray-400 dark:text-gray-400">
              Participate in Private ICOs on SAFE Launch.
            </p>
          </div>
          <button
            onClick={() => navigate("/safe-launch/private/create")}
                ...
                {currentLaunchpad.map((token, index) => (
                  ...
                  >
                    ...
                    <div className="text-sm text-gray-400 mt-2">
                      Token Price
                      <p className="flex mt-2 gap-2 items-center font-black text-xl text-safemoon-dark dark:text-white">
                        <img src={TonLogo} className="w-6 h-6 rounded-full" />{" "}
                        {token.token_price} TON{" "}
                        <small className="text-gray-400">
                          per {token.tokenInfo.symbol}
                        </small>
                      </p>
                    </div>
                    <div>
                      <div className="mt-2 pb-2 flex justify-between items-center border-b dark:border-gray-600">
                        Soft Cap <div>{token.soft_cap} TON</div>
                      </div>
                      <div className="mt-2 pb-2 flex justify-between items-center border-b dark:border-gray-600">
                        Hard Cap <div> {token.hard_cap} TON</div>
                      </div>
                    </div>
                    <div className="flex flex-col gap-2 mt-6">
                      <div className="flex flex-row justify-between items-center">
                        <div>
                          Progress (
                          {(Number(token.totalContributions) /
                            Number(token.hard_cap)) *
                            100}
                          %){" "}
                        </div>
                        <div className="flex gap-2 items-center">
                          <img src={TonLogo} className="w-4 h-4 rounded-full" />{" "}
                          {token.totalContributions} / {token.hard_cap} TON
                        </div>
                      </div>
                    ...
                    <div className="flex justify-between gap-4 mt-8 items-center">
                      <Link
                        to={`/safe-launch/private/${token._id}`}
